...
# Base recipe lr 2e-5 follows LLaVA-1.5 (Liu et al. 2024, arXiv:2310.03744). The
# length axis + low-LR-for-longer block cover the LR x length interaction.
#   0..1 length axis {800,1600} @ lr 2e-5   (~1 .. ~2.1 epochs)
#   2..5 low-LR-for-longer: {1e-5,5e-6} x {800,1600} steps
set -euo pipefail
mkdir -p logs
...
LRS=(2e-5 2e-5 1e-5 1e-5 5e-6 5e-6)
STS=(800  1600 800  1600 800  1600)
