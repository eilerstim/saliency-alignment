The downstream suite of \Cref{sec:metrics} comprises counting (CountBench~\citep{paiss2023teaching}), 2D/3D spatial reasoning (CV-Bench~\citep{tong2024cambrian}), general and fine-grained multimodal ability (MMStar~\citep{chen2024mmstar}, MMVP~\citep{tong2024mmvp}), low-level salient-object perception (the O3 split of SalBench~\citep{dahou2025salbench}), object hallucination (POPE~\citep{li2023pope}), and a blindness/bias probe (VLMs-are-Blind~\citep{rahmanzadehgervi2024blind}). This appendix expands the consolidated results (\Cref{tab:bench}) into the per-component, per-loss breakdown from our evaluation round, and adds measurements the main table omits: general visual question answering (GQA~\citep{hudson2019gqa}) and two further counting benchmarks (TallyQA~\citep{acharya2019tallyqa}, FSC-147~\citep{ranjan2021counting}). All numbers are zero-shot.
...
\begin{table}[h]
\centering
\caption{General VQA and hallucination (zero-shot). Higher is better; best per column in \textbf{bold}. Aligning the language model preserves capability; aligning the projector alone degrades it. $^{\star}$\,GQA and POPE both draw on COCO images.}
\label{tab:gqa_pope}
\vskip 0.1in
\small
\begin{tabular}{lccc}
\toprule
Variant & GQA acc.$^{\star}$ & POPE acc.$^{\star}$ & POPE F1$^{\star}$ \\
\midrule
...
\begin{table}[h]
\centering
\caption{TallyQA (which includes COCO images) counting accuracy (\%) by trained component and loss. Base (no fine-tuning) scores $40.96$. Training the projector helps counting; training the language model hurts it; the loss amplifies the component's direction.}
\label{tab:counting}
\vskip 0.1in
\small
\begin{tabular}{lcc}
